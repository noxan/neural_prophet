...
def data_params_definition(df, normalize, config_covariates=None, config_future_regressors=None, config_events=None):
    """
...
def init_data_params(
    ...
    global_data_params = data_params_definition(
        df_merged, normalize, config_covariates, config_future_regressors, config_events
    )
    ...
    for df_name, df_i in df.groupby("ID"):
        ...
        local_data_params[df_name] = data_params_definition(
            df_i, normalize, config_covariates, config_future_regressors, config_events
        )
...
def make_future_df(
    ...
    if config_future_regressors is not None:
        for regressor in regressors_df:
            # Todo: iterate over config_future_regressors instead
